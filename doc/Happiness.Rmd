...
# chosen <- sample(hm_data$hmid, 1000)
# hm_data <- subset(hm_data, hmid %in% chosen)
...
word <- c("happy","ago","yesterday","lot","today","months","month",
                 "happier","happiest","last","week","past", 
                  "time", "day", "finally", "morning", "night",
          "moment", "enjoyed")
...
## Classifying statements: happiness alone or with others 

```{r other VS friends}

# preprocessed words associated with being with others (family and friends)
source("wordlists.R")

library(stringr)
hm_data$others <- 0
for(i in 1:nrow(hm_data)){
  hm_data$others[i] <- sum(str_detect(hm_data$text[i], others))
}
